...
/* Definitions for task_CheckFlood */
osThreadId_t task_CheckFloodHandle;
const osThreadAttr_t task_CheckFlood_attributes = {
  .name = "task_CheckFlood",
  .stack_size = 256 * 4,
  .priority = (osPriority_t) osPriorityNormal,
};
/* Definitions for task_ControlOut */
osThreadId_t task_ControlOutHandle;
const osThreadAttr_t task_ControlOut_attributes = {
  .name = "task_ControlOut",
  .stack_size = 128 * 4,
  .priority = (osPriority_t) osPriorityNormal,
};
/* Definitions for task_ReadSensor */
...
osMessageQueueId_t floodQueueHandle;

osMessageQueueId_t tempQueueHandle;
osMessageQueueId_t humQueueHandle;
...
void StartTaskCheckFlood(void *argument);
void StartTaskControlOutputs(void *argument);
void StartTaskReadSensors(void *argument);
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
	...
	DWT_Init(); // Activează DWT pentru funcția delay_us

	floodQueueHandle = osMessageQueueNew(1, sizeof(uint8_t), NULL);

	tempQueueHandle = osMessageQueueNew(1, sizeof(float), NULL);
	humQueueHandle = osMessageQueueNew(1, sizeof(float), NULL);
  /* USER CODE END 2 */
  ...
  /* creation of task_CheckFlood */
  task_CheckFloodHandle = osThreadNew(StartTaskCheckFlood, NULL, &task_CheckFlood_attributes);

  /* creation of task_ControlOut */
  task_ControlOutHandle = osThreadNew(StartTaskControlOutputs, NULL, &task_ControlOut_attributes);

  /* creation of task_ReadSensor */
  task_ReadSensorHandle = osThreadNew(StartTaskReadSensors, NULL, &task_ReadSensor_attributes);
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOG, BUZZER_PIN_Pin|RELAY_PIN_Pin, GPIO_PIN_RESET);
  ...
  /*Configure GPIO pin : FLOOD_SENSOR_Pin */
  GPIO_InitStruct.Pin = FLOOD_SENSOR_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  HAL_GPIO_Init(FLOOD_SENSOR_GPIO_Port, &GPIO_InitStruct);
  ...
  /*Configure GPIO pins : BUZZER_PIN_Pin RELAY_PIN_Pin */
  GPIO_InitStruct.Pin = BUZZER_PIN_Pin|RELAY_PIN_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
...
/* USER CODE BEGIN Header_StartTaskCheckFlood */
/**
* @brief Function implementing the task_CheckFlood thread.
* @param argument: Not used
* @retval None
*/
/* USER CODE END Header_StartTaskCheckFlood */
void StartTaskCheckFlood(void *argument)
{
  /* USER CODE BEGIN StartTaskCheckFlood */
uint8_t last_status = 0;
uint8_t current_status = 0;
/* Infinite loop */
for(;;)
{
  // Cite?te pinul (presupunem HIGH = Flood Detectat)
  if (HAL_GPIO_ReadPin(FLOOD_SENSOR_GPIO_Port, FLOOD_SENSOR_Pin) == GPIO_PIN_SET)
  {
    current_status = 1; // Inunda?ie
  }
  else
  {
  ...
  // Trimite mesaj DOAR daca starea s-a schimbat
  if (current_status != last_status)
  {
    osMessageQueuePut(floodQueueHandle, &current_status, 0U, 0U);
    last_status = current_status;
  }

  osDelay(250); // Verifica de 4 ori pe secunda
}
  /* USER CODE END StartTaskCheckFlood */
}

/* USER CODE BEGIN Header_StartTaskControlOutputs */
/**
* @brief Function implementing the task_ControlOut thread.
* @param argument: Not used
* @retval None
*/
/* USER CODE END Header_StartTaskControlOutputs */
void StartTaskControlOutputs(void *argument)
{
  /* USER CODE BEGIN StartTaskControlOutputs */
uint8_t flood_status;
osStatus_t status;
/* Infinite loop */
for(;;)
{
  // A?teapta blocat un mesaj �n coada
  status = osMessageQueueGet(floodQueueHandle, &flood_status, NULL, osWaitForever);

  if (status == osOK) // Daca a primit un mesaj valid
  {
    if (flood_status == 1) // Mesaj de INUNDA?IE
    {
  ...
  /* USER CODE END StartTaskControlOutputs */
}

/* USER CODE BEGIN Header_StartTaskReadSensors */
/**
* @brief Function implementing the task_ReadSensor thread.
* @param argument: Not used
* @retval None
*/
/* USER CODE END Header_StartTaskReadSensors */
void StartTaskReadSensors(void *argument)
{
			/* USER CODE BEGIN StartTaskReadSensors */
			float temp, hum;
			/* Infinite loop */
			for(;;)
			{
				if (DHT22_Read(&temp, &hum) == 1) // Dacă citirea e cu succes
				{
					// Trimite noile valori în cozi
					// 'Put' va suprascrie valoarea veche dacă coada e plină (capacitate 1)
					osMessageQueuePut(tempQueueHandle, &temp, 0U, 0U);
					osMessageQueuePut(humQueueHandle, &hum, 0U, 0U);
				}

			
				osDelay(3000); 
			}
			/* USER CODE END StartTaskReadSensors */
}
